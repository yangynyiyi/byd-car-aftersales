...
public class AgentAssistantService {
    ...
    private static final String SCRIPT_SYSTEM_PROMPT =
            ...
            "要求：1) 全部使用中文，不要出现任何英文单词或字段名；2) 避免使用专业术语，用生活化语言；3) 说明故障原因和影响；4) 给出维修建议和注意事项；5) 语气亲切专业。" +
            "请严格以JSON格式返回，字段名也必须使用中文：{\"故障解释\":\"...\",\"维修建议\":\"...\",\"注意事项\":\"...\"}。" +
            "只返回纯JSON文本，不要使用markdown代码块包裹，不要返回任何其他内容。";

    private static final String QUOTE_SYSTEM_PROMPT =
            "你是一位比亚迪售后维修报价专家。" +
            "请根据诊断结果、推荐检测项目和备件价格，生成一份维修报价单。" +
            "要求：1) 全部使用中文，不要出现任何英文单词或字段名；2) 列出可能需要的备件及预估价格；3) 给出工时费估算；4) 给出总价区间。" +
            "请严格以JSON格式返回，字段名也必须使用中文：{\"费用明细\":[{\"项目名称\":\"...\",\"类型\":\"备件/工时\",\"预估价格\":数字}],\"工时费估算\":数字,\"最低总价\":数字,\"最高总价\":数字,\"报价说明\":\"...\"}。" +
            "只返回纯JSON文本，不要使用markdown代码块包裹，不要返回任何其他内容。";

    private static final String MAINTENANCE_SYSTEM_PROMPT =
            "你是一位比亚迪新能源汽车保养专家。" +
            "请根据车辆里程、电池健康度、使用习惯等信息，生成个性化保养计划建议。" +
            "要求：1) 全部使用中文，不要出现任何英文单词或字段名；2) 按紧急程度排序；3) 说明每项保养的原因和建议时间。" +
            "请严格以JSON格式返回，字段名也必须使用中文：{\"保养建议\":[{\"保养项目\":\"...\",\"紧急程度\":\"紧急/重要/建议\",\"原因\":\"...\",\"建议时间\":\"...\"}],\"总结\":\"...\"}。" +
            "只返回纯JSON文本，不要使用markdown代码块包裹，不要返回任何其他内容。";
    ...
    private String buildLocalFallback(String systemPrompt, String userMessage) {
        if (systemPrompt.contains("售后服务顾问")) {
            return "{\"故障解释\":\"根据诊断结果，您的车辆存在需要关注的故障情况，建议尽快安排检修以确保行车安全。\",\"维修建议\":\"建议尽快到比亚迪授权服务中心进行全面检测和维修。\",\"注意事项\":\"维修期间请勿强行使用车辆，以免造成更大损失。\"}";
        } else if (systemPrompt.contains("保养专家")) {
            return "{\"保养建议\":[{\"保养项目\":\"常规保养检查\",\"紧急程度\":\"建议\",\"原因\":\"定期保养可延长车辆使用寿命\",\"建议时间\":\"按保养手册周期\"},{\"保养项目\":\"电池健康检测\",\"紧急程度\":\"重要\",\"原因\":\"确保电池性能和安全\",\"建议时间\":\"每半年一次\"}],\"总结\":\"建议按保养手册定期保养，关注电池健康状态。\"}";
        } else {
            return "{\"费用明细\":[{\"项目名称\":\"检测费\",\"类型\":\"工时\",\"预估价格\":200},{\"项目名称\":\"备件费（预估）\",\"类型\":\"备件\",\"预估价格\":500}],\"工时费估算\":200,\"最低总价\":500,\"最高总价\":1000,\"报价说明\":\"以上为预估价格，实际费用以服务中心报价为准。\"}";
        }
    }
}
